...
function flashBtn(id, defaultText, cls) {
  const b = $(id); b.textContent = "✓ Saved!"; b.classList.add("saved");
...
function renderDashboard() {
  ...
  if (!S.quoteDismissed) {
    ...
    $("quote-banner-wrap").innerHTML = `<div class="quote-banner"><span class="quote-text">"${q}"</span><button class="quote-dismiss" onclick="dismissQuote()">x</button></div>`;
  ...
  $("phase-countdown-grid").innerHTML = PHASES.map(p => {
    ...
    return `<div class="phase-cd-card${isActive ? " active-phase" : ""}">
      ...
      <div class="phase-cd-days" style="color:${isPast ? "#c4a0b0" : p.color}">${isPast ? "Day : " + daysN : ""}</div>
  ...
  $("last-mock-content").innerHTML = last ? `
    ...
    <div class="mock-scores-grid">
      ${[["QA", last.qa, "#F472B6"], ["VARC", last.varc, "#FB923C"], ["DILR", last.dilr, "#10B981"]].map(([s, v, c]) => `<div class="mock-score-box"><div class="label">${s}</div><div class="value" style="color:${c}">${v ?? 'â€”'}</div></div>`).join("")}
  ...
  $("recent-logs-content").innerHTML = S.logs.length ? S.logs.slice(-5).reverse().map(l => `
    <div class="recent-log-row">
      ...
      <div class="recent-log-breakdown"><span style="color:#F472B6">QA ${l.qa}m</span> <span style="color:#FB923C">VARC ${l.varc}m</span> <span style="color:#10B981">DILR ${l.dilr}m</span></div></div>
...
function renderMocks() {
  ...
  $("mock-history-list").innerHTML = [...S.mocks].reverse().map((m, i) => {
    ...
    </div><button class="btn-delete" onclick="removeMock(${ri})">x</button></div>`;
  }).join("");
}
...
function renderErrors() {
  ...
  $("error-log-list").innerHTML = [...S.errors].reverse().map((e, i) => {
    ...
    </div><button class="btn-delete" onclick="removeError(${ri})">x</button></div>`;
  }).join("");
}
...
function calcScore() {
  ...
  if (!pct || pct < 0 || pct > 100) { alert("Enter a valid percentile (0â€“100)."); return; }
